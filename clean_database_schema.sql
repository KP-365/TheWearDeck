-- Clean FashionBrain Database Schema - DROP AND RECREATE
...
DROP TABLE IF EXISTS public.inspo_images CASCADE;
...
-- Step 6: Create user_actions table
...
-- Step 7: Create inspo_images table
CREATE TABLE public.inspo_images (
    id uuid NOT NULL DEFAULT gen_random_uuid(),
    user_id uuid,
    image_url text NOT NULL,
    cloudinary_public_id text,
...
CREATE INDEX inspo_images_embedding_idx 
ON inspo_images 
USING ivfflat (embedding vector_l2_ops) 
WITH (lists = 100);

CREATE INDEX user_actions_user_id_idx ON user_actions(user_id);
CREATE INDEX user_actions_product_id_idx ON user_actions(product_id);
CREATE INDEX inspo_images_user_id_idx ON inspo_images(user_id);

-- Step 10: Create the similarity search function
