-- Complete FashionBrain Database Schema (Fixed for jsonb conversion)
...
-- Step 3: Create products table (without embedding first)
...
-- Step 4: Add embedding column to products (convert from jsonb if exists)
DO $$ 
BEGIN
    -- Check if embedding column exists and is jsonb
    IF EXISTS (
...
-- Step 5: Create user_actions table
...
-- Step 6: Create inspo_images table
...
-- Step 8: Create indexes for better performance (only if embedding column exists and is vector type)
DO $$ 
BEGIN
    -- Check if products embedding column is vector type
    IF EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'products' 
        AND column_name = 'embedding' 
...
-- Step 9: Create the similarity search function
